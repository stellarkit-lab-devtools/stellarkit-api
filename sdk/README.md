...
| | `getEffects(id, {limit?, cursor?, type?, order?, fresh?})` | Paginated ledger effects (Effect {effectId, type, createdAt, asset?, amount?}) — `GET /account/:id/effects` (fresh bypasses cache) |
